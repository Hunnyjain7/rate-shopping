...
from core.utils import create_client_name, get_authenticated_user
...
class ClientSerializer(serializers.ModelSerializer):
    ...
    profile_image = serializers.FileField(
        write_only=True, required=False, allow_null=True, allow_empty_file=True
    )
    ...
    def get_address_data(validated_data, instance=None):
        ...
        return {
            field: validated_data.pop(field, getattr(instance, field, None))
            for field in fields
    ...
    def create(self, validated_data):
        ...
        user_instance = user_serializer.save(
            user_type_term=CLIENT, association_type_term=CLIENT
        )
    ...
    def update(self, instance, validated_data):
        ...
        user_serializer = UsrUserSerializer(
            user_instance, data=user_data, context=self.context, partial=True
        )
        user_serializer.is_valid(raise_exception=True)
