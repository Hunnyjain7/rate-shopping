...
class Subscription(BaseActiveDeleteModel):
    ...
    subscription_image = models.FileField(upload_to="media/images/subscription_images", null=True, blank=True)
    ...
    order_number = models.PositiveIntegerField(unique=True, editable=False, serialize=True, auto_created=True)
...
class ClientSubscription(BaseActivationRenewal):
    ...
    invoice_ref_number = models.CharField(max_length=39, default=f"INV-REF-{get_random_number()}", editable=False)
    is_active = models.BooleanField(default=False)
    update_log = models.DateTimeField(auto_now=True)
    ...
    ref_client_subscription = models.ForeignKey('self', default=None, null=True, on_delete=models.DO_NOTHING)
...
class ClientSubscriptionDetail(MaxCCP):
    ...
    client = models.ForeignKey(Client, on_delete=models.DO_NOTHING)
    subscription = models.ForeignKey(Subscription, on_delete=models.DO_NOTHING)
    client_subscription = models.ForeignKey(ClientSubscription, on_delete=models.DO_NOTHING)
